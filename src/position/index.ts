...
export function createPositionManager(config: PositionManagerConfig): AntiMartingalePositionManager | FixedPositionManager {
  switch (config.type) {
    case 'anti_martingale':
      return new AntiMartingalePositionManager(config.params as Partial<AntiMartingaleConfig>);
    
    case 'fixed':
      const fixedParams = config.params as { fixedSize?: number; floorMultiplier?: number } | undefined;
