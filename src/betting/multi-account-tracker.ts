...
export class MultiAccountTracker {
  ...
  constructor(config: BettingStrategyConfig) {
    ...
    for (const target of config.takeProfitTargets) {
      this.accounts.set(target, new VirtualAccount(target, this.enableRiskControl));
    }
  }
  ...
  processTradeResult(
    ...
    for (const [target, account] of this.accounts) {
      account.processTradeResult(pnlPercent, candleIndex, tradeIndex, externalC, externalStopLoss);
