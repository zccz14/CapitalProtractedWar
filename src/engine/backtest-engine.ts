 ...
 * 计算盈亏百分比
 */
function calculatePnL(position: number, entryPrice: number, exitPrice: number): number {
 ...
 * 评估单个信号策略在给定市场上的表现
 */
export function evaluateSignalStrategy(
  candles: Candle[],
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    if (pendingSignal !== null && pendingSignalIndex !== null) {
      ...
      if (
        ...
        const pnlPercent = calculatePnL(currentPosition, entryPrice, executionPrice);
  ...
  if (
    ...
    const pnlPercent = calculatePnL(currentPosition, entryPrice, exitPrice);
