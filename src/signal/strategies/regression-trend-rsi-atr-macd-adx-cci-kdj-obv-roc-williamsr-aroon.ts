/**
 * 线性回归趋势 + RSI + ATR + MACD + ADX + CCI + KDJ + OBV + ROC + Williams %R + Aroon 门控策略
 *
 * 在 Williams %R 基础上加入 Aroon 指标：
 * - Aroon 衡量周期内最高价/最低价的新鲜度
 * - Aroon Up > 70 表示强劲上升趋势
 * - Aroon Down < 30 表示强劲下降趋势
 * - Aroon 交叉可用于识别趋势转折点
 */
...
export interface RegressionTrendRsiAtrMacdAdxCciKdjObvRocWilliamsrAroonParams {
  /** 回归周期 (默认24) */
  ...
  aroonPeriod: number;
  /** Aroon Up 超强阈值 (默认70) */
  aroonUpThreshold: number;
  /** Aroon Down 超弱阈值 (默认30) */
  aroonDownThreshold: number;
}

const DEFAULT_PARAMS: RegressionTrendRsiAtrMacdAdxCciKdjObvRocWilliamsrAroonParams = {
  lookbackPeriod: 24,
  ...
  minAdx: 20,
  cciPeriod: 20,
  cciThreshold: 50, // 放宽阈值
  kdjPeriod: 9,
  kdjKPeriod: 3,
  kdjDPeriod: 3,
  kdjOverbought: 70, // 放宽阈值
  kdjOversold: 30, // 放宽阈值
  obvLookback: 20,
  ...
  aroonPeriod: 25,
  aroonUpThreshold: 70,
  aroonDownThreshold: 30,
};

@Strategy({
  type: 'regression_trend_rsi_atr_macd_adx_cci_kdj_obv_roc_williamsr_aroon',
  name: '回归趋势+RSI+ATR+MACD+ADX+CCI+KDJ+OBV+ROC+WilliamsR+Aroon门控',
  description:
    '在回归斜率+RSI+ATR+MACD+ADX+CCI+KDJ+OBV+ROC+WilliamsR基础上加入Aroon趋势强度确认，提供额外趋势过滤层',
  category: 'trend',
  ...
  paramDescriptions: {
    ...
    aroonUpThreshold: 'Aroon Up 强劲上升阈值',
    aroonDownThreshold: 'Aroon Down 强劲下降阈值',
  },
})
export class RegressionTrendRsiAtrMacdAdxCciKdjObvRocWilliamsrAroonStrategy extends BaseStrategy<RegressionTrendRsiAtrMacdAdxCciKdjObvRocWilliamsrAroonParams> {
  readonly type: SignalStrategyType =
    'regression_trend_rsi_atr_macd_adx_cci_kdj_obv_roc_williamsr_aroon';

  constructor(params?: Partial<RegressionTrendRsiAtrMacdAdxCciKdjObvRocWilliamsrAroonParams>) {
    super({
      ...DEFAULT_PARAMS,
      ...params,
    });
  ...
  generate(candles: Candle[], currentIndex: number): Signal {
    ...
    const aroonValue = aroon(candles, currentIndex, aroonPeriod);
    const aroonBullish = aroonValue.aroonUp >= aroonUpThreshold;
    const aroonBearish = aroonValue.aroonDown <= aroonDownThreshold;
