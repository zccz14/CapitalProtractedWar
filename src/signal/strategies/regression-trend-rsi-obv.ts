 ...
 * 线性回归趋势 + RSI + OBV 确认策略
 *
 * 双指标：回归斜率 + RSI 动量确认 + OBV 资金流趋势过滤
 */

import type { Candle, Signal, SignalStrategyType } from '../../types.js';
import { BaseStrategy, Strategy } from '../base.js';
import { linearRegressionSlope, obv, rsi, volumeProxy } from '../indicators/index.js';

export interface RegressionTrendRsiObvParams {
  /** 回归周期 (默认20) */
...
const DEFAULT_PARAMS: RegressionTrendRsiObvParams = {
  lookbackPeriod: 20,
...
@Strategy({
  type: 'regression_trend_rsi_obv',
  name: '回归趋势+RSI+OBV',
  description: '回归斜率与RSI动量确认，并要求OBV资金流趋势一致',
  category: 'trend',
...
export class RegressionTrendRsiObvStrategy extends BaseStrategy<RegressionTrendRsiObvParams> {
  readonly type: SignalStrategyType = 'regression_trend_rsi_obv';

  constructor(params?: Partial<RegressionTrendRsiObvParams>) {
    super({
      ...DEFAULT_PARAMS,
      ...params,
    });
  ...
  generate(candles: Candle[], currentIndex: number): Signal {
    ...
    const required = Math.max(lookbackPeriod, rsiPeriod, obvLookback);
