/**
 * 线性回归趋势 + RSI + ATR + MACD + ADX + CCI + KDJ + OBV 门控策略
 *
 * 在 KDJ 门控基础上加入 OBV 成交量趋势确认：
 * - OBV 通过累积成交量代理衡量趋势资金流
 * - 当 OBV 斜率与趋势方向一致时才开仓，降低无量突破
 */
...
export interface RegressionTrendRsiAtrMacdAdxCciKdjObvParams {
  /** 回归周期 (默认24) */
...
const DEFAULT_PARAMS: RegressionTrendRsiAtrMacdAdxCciKdjObvParams = {
  lookbackPeriod: 24,
...
@Strategy({
  type: 'regression_trend_rsi_atr_macd_adx_cci_kdj_obv',
  name: '回归趋势+RSI+ATR+MACD+ADX+CCI+KDJ+OBV门控',
  description: '在回归斜率+RSI+ATR+MACD+ADX+CCI+KDJ基础上加入OBV趋势确认，降低无量突破噪声',
  category: 'trend',
...
export class RegressionTrendRsiAtrMacdAdxCciKdjObvStrategy extends BaseStrategy<RegressionTrendRsiAtrMacdAdxCciKdjObvParams> {
  readonly type: SignalStrategyType = 'regression_trend_rsi_atr_macd_adx_cci_kdj_obv';

  constructor(params?: Partial<RegressionTrendRsiAtrMacdAdxCciKdjObvParams>) {
    super({
      ...DEFAULT_PARAMS,
      ...params,
    });
