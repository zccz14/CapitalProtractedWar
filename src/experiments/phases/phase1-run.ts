...
import type { MarketConfig, SignalStrategyConfig } from '../../types.js';
...
export async function runPhase1(options: Phase1Options): Promise<Phase1Result> {
  ...
  const totalCombinations = config.volatilities.length * config.drifts.length * monteCarloRuns * config.signals.length;
  let processed = 0;
      ...
      for (let seedOffset = 0; seedOffset < monteCarloRuns; seedOffset++) {
        ...
        for (const signalConfig of config.signals) {
          ...
          if (isRunCacheValid(runPath, configHash, force)) {
            cachedRuns++;
            if (verbose) {
              console.log(`  [${processed}/${totalCombinations}] 缓存命中: ${marketId}/${signalId}`);
            }
            continue;
          }
