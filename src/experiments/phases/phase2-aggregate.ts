...
import type { FullExperimentConfig, AggregatedResult, RunStats, SampleIndex } from '../../cache/types.js';
...
function aggregateRuns(
  ...
  const takeProfitStats = Array.from(targetSet).sort((a, b) => a - b).map(target => {
    // 收集所有间隔
...
function selectRepresentativeSamples(
  runs: { marketId: string; stats: RunStats }[]
): { best: SampleIndex; median: SampleIndex; worst: SampleIndex } {
